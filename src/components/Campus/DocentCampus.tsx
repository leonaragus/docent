...
import { subscribeToSharedClasses, loadAcademiesFromDb, saveAcademiesToDb } from '../../supabase';
import { 
  INITIAL_COURSES, 
  INITIAL_TALLERES, 
  INITIAL_FEED, 
...
import { Course, TallerVivo, CommunityPost, UserState, Module, Academy, RecordedClass } from './campusTypes';
...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToSharedClasses((classes) => {
      setSupabaseClasses(classes);
    });
    
    // Sync academies from Supabase on mount
    loadAcademiesFromDb().then(dbAcademies => {
      if (dbAcademies && dbAcademies.length > 0) {
        setAcademies(dbAcademies);
        localStorage.setItem('docent_academies_list', JSON.stringify(dbAcademies));
      }
    }).catch(err => {
      console.warn("Could not sync academies from Supabase on mount:", err);
    });
  ...
  // Initialize unified list of academies from cache or defaults (docent Certified Academies)
  const [academies, setAcademies] = useState<Academy[]>(() => {
    try {
      const cached = typeof window !== 'undefined' ? localStorage.getItem('docent_academies_list') : null;
      if (cached) {
      ...
      console.error("Error parsing cached academies list:", e);
    }
    ...
    return [
      {
        id: 'diseno',
        subdomain: 'diseno',
        name: isEn ? 'DOCENT Technology and Trades Institute' : 'Instituto de Tecnología y Oficios docent',
        description: isEn ? 'Approved model pilot center for teaching technological courses and non-formal disciplines with institutional endorsement.' : 'Centro piloto modelo homologado para el dictado de cursos tecnológicos y disciplinas no formales con aval institucional.',
        themeId: 'indigo',
  ...
  const [activeAcademyId, setActiveAcademyId] = useState<string>(() => {
    const cached = typeof window !== 'undefined' ? localStorage.getItem('docent_active_academy_id') : null;
    return cached || 'diseno';
  });

  // Track the active selected course ID to support dynamic multi-course dashboards
  const [activeCourseId, setActiveCourseId] = useState<string | null>(() => {
    const cached = typeof window !== 'undefined' ? localStorage.getItem('docent_active_course_id') : null;
    return cached;
  });

  // Derived active academy data block dynamically with complete safety fallbacks
  ...
  const activeAcademy = (academies && academies.length > 0)
  ...
  const activeTheme = THEME_PRESETS.find(t => t.id === (activeAcademy?.themeId || 'indigo')) || THEME_PRESETS[0];

  const activeCourse = (courses && courses.length > 0)
    ? (courses.find(c => c.id === activeCourseId) || courses[0])
    : null;

  // Merge live Supabase classes into recordings
  const dbRecordedClasses = supabaseClasses.map(sc => ({
    id: sc.id,
    title: sc.classTitle,
    youtubeUrl: sc.videoUrl, // plays Drive or Youtube inside player
    transcription: sc.srtText || '',
    duration: sc.duration || '0:00',
    date: sc.createdAt ? new Date(sc.createdAt).toLocaleDateString() : 'Recent'
  }));
  const allRecordedClasses = [...dbRecordedClasses, ...(activeCourse?.recordedClasses || [])];
  ...
  const updateActiveAcademy = (updater: (academy: Academy) => Academy) => {
    ...
    localStorage.setItem('docent_academies_list', JSON.stringify(updatedList));
    
    // Sync to Supabase in the background
    saveAcademiesToDb(updatedList).catch(err => {
      console.warn("Offline/local mode fallback. Supabase campus_academies write skipped:", err.message);
    });
  };
  ...
  const handleSelectAcademy = (id: string) => {
    ...
    localStorage.setItem('docent_active_academy_id', id);
  ...
  const handleCreateAcademy = (newAcad: { name: string; subdomain: string; themeId: string; creatorName: string; logoUrl?: string }) => {
    const newAcademy: Academy = {
      ...
      name: newAcad.name,
      description: isEn ? `Synchronous and non-formal virtual school of ${newAcad.name}` : `Escuela virtual sincrónica y no formal de ${newAcad.name}`,
    ...
    localStorage.setItem('docent_academies_list', JSON.stringify(updatedList));
    localStorage.setItem('docent_active_academy_id', newAcademy.id);
  ...
  const handleDeleteAcademy = (id: string) => {
    ...
    if (activeAcademyId === id) {
      ...
      localStorage.setItem('docent_active_academy_id', fallbackId);
    }
  };
  ...
  const handleAddPost = (text: string, category: CommunityPost['category'], image?: string) => {
    const newPost: CommunityPost = {
      ...
      text,
      timestamp: isEn ? 'Just now' : 'Ahora mismo',
      image,
      category,
  ...
  const handleSubmitChallenge = (moduleId: string, submissionText: string, link: string) => {
    ...
    const newSubmission = {
      ...
      submittedAt: isEn ? 'A few moments ago' : 'Hace unos instantes',
    ...
    setTimeout(() => {
      updateActiveAcademy(currentAcad => {
        ...
        updatedSubs[subIndex] = {
          ...currentSub,
          status: 'reviewed',
          feedback: isEn ? `[Special Synchronous Evaluation]: Excellent submission! Meets the visual harmony and mass balance guidelines. The provided link works flawlessly.` : `[Evaluación Sincrónica Especial]: ¡Excelente entrega! Cumple con la pauta de armonía visual y balance de masas. El enlace provisto funciona impecablemente.`,
          gradeBadge: 'Oro',
          xpAwarded: 200
        };
  ...
  const handleResetData = () => {
    localStorage.removeItem('docent_academies_list');
    localStorage.removeItem('docent_active_academy_id');
    localStorage.removeItem('docent_active_course_id');
    const initial = [
      {
        id: 'diseno',
        subdomain: 'diseno',
    ...
    saveAcademiesToDb(initial).catch(e => {});
  ...
  return (
    <div className="min-h-screen bg-[#FBFBFC] text-slate-805 antialiased pb-12">
      ...
      {/* GLOBAL OFFICIAL BANNER (docent ACCREDITATION SYSTEM) */}
      <div className={`bg-gradient-to-r from-slate-900 to-indigo-950 text-white text-center py-2.5 px-4 shadow-sm text-xs font-semibold flex flex-col sm:flex-row items-center justify-center gap-3 relative font-sans`}>
        <span className="bg-amber-500 text-slate-950 px-2 py-0.5 rounded-full text-[9px] font-black tracking-wider uppercase">{isEn ? 'DOCENT Endorsement Seal' : 'Sello de Homologación docent'}</span>
        <span>{isEn ? 'Accreditation from the Argentine Registry of Institutional Homologation and Professional Certification' : 'Acreditación del Registro Argentino de Homologación Institucional y Certificación Profesional'}</span>
        <button 
        ...
        >
          {isEn ? 'Reset Academy Network' : 'Resetear Red de Academias'}
        </button>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-6">
        ...
        <header className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-6 pb-6 mb-2 border-b border-slate-100">
          ...
          {/* OFICIAL docent INSTITUTIONAL LOGO */}
          ...
          <div className="flex flex-wrap items-center gap-4 bg-white p-3 rounded-2xl border border-slate-100 shadow-xs">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200/60 rounded-xl">
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider">{isEn ? 'Language:' : 'Idioma:'}</span>
              <select
                value={lang}
                onChange={(e) => setLang(e.target.value as any)}
                className="bg-transparent text-xs font-bold text-slate-700 outline-none cursor-pointer uppercase border-0 p-0"
              >
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="text-left leading-none">
                  ...
                  <span className="text-[9px] text-slate-450 font-medium font-sans uppercase">
                    {activeRole === 'teacher' ? (isEn ? 'Teacher / Manager 🛡️' : 'Docente / Gestor 🛡️') : (isEn ? 'Private Access' : 'Acceso Privado')}
                  </span>
                </div>
              </div>
        ...
        <nav className="flex flex-wrap gap-2 p-1.5 bg-slate-110/80 backdrop-blur-md rounded-2xl border border-slate-200/40 mb-2 max-w-fit">
          {[
            { id: 'dashboard', label: isEn ? 'My Dashboard' : 'Mi Panel de Control', icon: Compass },
            { id: 'journey', label: isEn ? 'Learning Journey (Map)' : 'Ruta de Aprendizaje (Map)', icon: GraduationCap },
            { id: 'evaluations', label: isEn ? 'Evaluations' : 'Evaluaciones', icon: CheckCircle2 },
            { id: 'recordings', label: isEn ? 'Recorded Classes' : 'Clases Grabadas', icon: Clock },
            { id: 'feed', label: isEn ? 'Social Pulse Wall' : 'El Muro Social de Pulso', icon: MessageSquare },
            { id: 'tutor', label: isEn ? 'Kairós AI Assistant' : 'Asistente IA Kairós', icon: Sparkles },
            ...(activeRole === 'teacher' ? [{ id: 'creator', label: isEn ? 'Course Builder 🛠️' : 'Constructor de Cursados 🛠️', icon: Wrench }] : [])
          ].map(tab => {
          ...
          >
                ...
                {courses.length === 0 ? (
                  <>
                    <h4 className="text-medium text-xs font-black text-slate-900 font-display uppercase tracking-wider">
                      {isEn ? 'Your New Network Academy is Fresh, Empty and Ready!' : '¡Tu Nueva Academia de la Red está Nueva, Vacía y Lista!'}
                    </h4>
                    <p className="text-slate-650 text-xs leading-relaxed max-w-4xl font-sans">
                      {isEn ? 'You have created the simulated academy' : 'Has creado la academia'} <span className="font-bold text-slate-900">&ldquo;{activeAcademy.name}&rdquo;</span> {isEn ? 'at' : 'simulada en'} <span className="font-bold font-mono text-indigo-600">https://{activeAcademy.subdomain}.docent.app</span>.
                      {isEn ? 'Since it has no courses yet, follow these steps to start your own academic curriculum:' : 'Como no tiene ningún curso de momento, sigue estos pasos para iniciar tu propio pensum académico:'}
                      <br />
                      {isEn ? '1. Activate the role of **Teacher / Manager 🛡️** above in the corporate role selector.' : '1. Activa el rol de **Docente / Gestor 🛡️** arriba en el selector de roles corporativos.'}
                      <br /> 
                      {isEn ? '2. You will see the special tab **"Course Builder 🛠️"** enabled. Click there.' : '2. Verás habilitarse el tab especial **&ldquo;Constructor de Cursados 🛠️&rdquo;**. Haz clic allí.'}
                      <br />
                      {isEn ? '3. Press **"+ Create New Course"**, set a title, and add modules, syllabus, technical challenges, or schedule in-person/virtual workshops with total freedom.' : '3. Presiona **&ldquo;+ Crear Nuevo Cursado&rdquo;**, configúrale un título, y ve agregando módulos, syllabus, desafíos técnicos, o programa talleres presenciales/virtuales con total libertad.'}
                    </p>
                  </>
                ) : (
                  <>
                    <h4 className="text-medium text-xs font-black text-slate-900 font-display uppercase tracking-wider">
                      {isEn ? 'Navigation Center:' : 'Centro de Navegación:'} {activeAcademy.name}
                    </h4>
                    <p className="text-slate-650 text-xs leading-relaxed max-w-4xl font-sans">
                      {isEn ? 'You are browsing the academy' : 'Estás navegando en la academia'} <span className="font-bold text-slate-900">{activeAcademy.name}</span>. 
                      {isEn ? 'To simulate or create a parallel academy with isolated colors or curricula, click **"Change or Create Academy"** in the top selector.' : 'Para simular o crear una academia paralela con colores o currículos aislados, haz clic en **&ldquo;Cambiar o Crear Academia&rdquo;** en el selector superior.'}
                      {isEn ? 'Follow the route, compete for XP by completing milestones, and chat with our Tutor Kairós.' : 'Sigue la ruta, compite por XP completando hitos, y chatea con nuestro Tutor Kairós.'}
                    </p>
                  </>
                )}
              </div>
            ...
            <div className="shrink-0 flex items-center self-center bg-white border border-slate-200 rounded-2xl px-3.5 py-2">
              <span className={`text-[10px] uppercase font-bold tracking-wider ${activeTheme.accentText}`}>
                {isEn ? 'Identity:' : 'Identidad:'} {activeTheme.name}
              </span>
            </div>
          </motion.div>
        </AnimatePresence>
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <Dashboard 
                  ...
                  onSelectCourse={(courseId) => {
                    ...
                    localStorage.setItem('docent_active_course_id', courseId);
                  }}
            ...
            {activeTab === 'recordings' && (
              <motion.div
                ...
                transition={{ duration: 0.2 }}
              >
                <RecordedClasses classes={allRecordedClasses} />
              </motion.div>
            )}
            ...
            {activeTab === 'tutor' && (
              ...
              >
                <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-4 mb-4 flex gap-3 items-start max-w-4xl">
                  ...
                  <div>
                    <h5 className="text-xs font-bold text-indigo-900 font-sans">{isEn ? 'End-to-End Connected AI' : 'IA Conectada de Extremo a Extremo'}</h5>
                    <p className="text-[11px] text-indigo-800 leading-normal mt-0.5">
                      {isEn ? 'This copilot is real and connected to backend servers. It communicates with the Google AI Studio **gemini-3.5-flash** model. Try asking it for an interactive test in Spanish.' : 'Este copiloto es real y está conectado a los servidores del backend. Se comunica con el modelo **gemini-3.5-flash** de Google AI Studio. Pruébalo pidiéndole un test interactivo en español.'}
                    </p>
                  </div>
                </div>
                <AiTutor />
        ...
        <footer className="mt-16 pt-8 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px] text-slate-400 font-sans">
          <p>{isEn ? '© 2026 docent Institutional Homologation and Professional Certification System. Private Collaborative Co-learning Space.' : '© 2026 docent Sistema de Homologación Institucional y Certificación Profesional. Espacio de Co-aprendizaje Colaborativo Privado.'}</p>
          <div className="flex items-center gap-4">
            <span className="hover:text-slate-600 transition-colors">{isEn ? 'Community Guidelines' : 'Normas de la Comunidad'}</span>
            <span className="w-1.5 h-1.5 rounded-full bg-slate-200" />
            <span className="hover:text-slate-600 transition-colors">{isEn ? 'Support Center' : 'Centro de Soporte'}</span>
