...
export default function FlyerDesigner({
  ...
  const BACKGROUNDS = [
    ...
    { name: "Monochrome Pure Blanco", class: "from-white via-slate-50 to-white text-white", border: "border-slate-800" }
  ];
  ...
  const colorClasses = {
    ...
    white: "text-slate-300 bg-slate-9500/10 border-slate-500/20"
  };
  ...
  const getSubtleTextColor = () => {
    return activeBg.name === "Monochrome Pure Blanco" ? "text-slate-400" : "text-slate-400";
  };

  const getTitleColor = () => {
    return activeBg.name === "Monochrome Pure Blanco" ? "text-white" : "text-white";
  };
  ...
  return (
    <div id="flyer-designer-container" className="flex flex-col gap-6 text-left">
      ...
      <div className="bg-slate-950 border border-slate-800 p-5 rounded-xl flex items-start gap-3.5 shadow-2xs">
        ...
        <div>
          <h4 className="text-sm font-bold text-slate-100 tracking-tight">Creador de Flyers y Storyboarding para Mini Video Publicitario</h4>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
        ...
        <div className="xl:col-span-5 flex flex-col gap-5">
          ...
          <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl shadow-2xs">
          ...
          <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl shadow-2xs flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] uppercase font-bold text-slate-400">Texto del Badge Promocional</label>
              <input
                type="text"
                value={badgeText}
                onChange={(e) => setBadgeText(e.target.value.toUpperCase())}
                className="w-full px-3 py-1.5 bg-slate-950 border border-slate-800 rounded-md text-xs text-slate-100 font-semibold"
              />
              <div className="flex flex-wrap gap-1 mt-1">
                {BADGE_PRESETS.map((p, i) => (
                  <button
                    ...
                    className="text-[9px] bg-slate-950 hover:bg-slate-800 px-2 py-1 rounded border border-slate-800 text-slate-400 cursor-pointer font-medium"
                  >
                    {p}
                  </button>
                ))}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] uppercase font-bold text-slate-400">Título para el Canvas</label>
              <textarea
                rows={2}
                value={customHeading}
                onChange={(e) => setCustomHeading(e.target.value)}
                className="w-full px-3 py-1.5 bg-slate-950 border border-slate-800 rounded-md text-xs text-slate-100 font-medium resize-none"
              />
            </div>

            {/* Custom Input: Custom Detail sentence */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] uppercase font-bold text-slate-400">Detalles / Beneficios del Certificado</label>
              <input
                type="text"
                value={customDetail}
                onChange={(e) => setCustomDetail(e.target.value)}
                className="w-full px-3 py-1.5 bg-slate-950 border border-slate-800 rounded-md text-xs text-slate-100"
              />
            </div>

            {/* Custom Palette selection for Glow effects */}
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] uppercase font-bold text-slate-400">Color Destacado</label>
                ...
                {(["indigo", "rose", "emerald", "amber", "white"] as const).map((color) => (
                  <button
                    ...
                    className={`w-6 h-6 rounded-full border cursor-pointer flex items-center justify-center text-[10px] uppercase font-bold ${
                      highlightColor === color ? "border-slate-800 scale-110 shadow-[0_0_15px_rgba(0,0,0,0.5)]" : "border-slate-800"
                    }`}
                    style={{
                      backgroundColor:
                        color === "indigo" ? "#6366f1" :
          ...
          <div className="bg-slate-800 p-4 rounded-xl border border-slate-250 flex flex-col gap-2.5">
            <span className="text-xs font-bold text-slate-300 block">📥 Cómo usar este Flyer en tus campañas:</span>
        ...
        <div className="xl:col-span-7 flex flex-col gap-6">
          ...
          <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl">
            ...
            <div
              ...
              className={`aspect-square w-full rounded-xl p-8 flex flex-col justify-between relative overflow-hidden bg-gradient-to-br border shadow-[0_0_20px_rgba(0,0,0,0.5)] select-none ${activeBg.class} ${activeBg.border}`}
            >
              ...
              {activeBg.name !== "Monochrome Pure Blanco" && (
                <div 
                  className={`absolute bottom-[-15%] right-[-10%] w-60 h-60 rounded-full filter blur-3xl opacity-35 mix-blend-screen transition-all ${
                    ...
                    highlightColor === "amber" ? "bg-amber-600" : "bg-slate-9500"
                  }`}
                />
              )}
              ...
              <div className="flex justify-between items-end border-t border-slate-800/10 pt-4 z-10 mt-auto">
                ...
                <span className="text-xs bg-slate-900 border border-slate-800/50 hover:bg-slate-800 font-bold px-3.5 py-2 rounded text-white tracking-wider flex-none uppercase shadow-[0_0_15px_rgba(0,0,0,0.5)]">
                  Inscribirse
                </span>
              </div>
          ...
          <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl shadow-2xs">
            <div className="flex items-center gap-2 mb-4">
              <div className="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center">
                <Video size={16} />
              </div>
              <div>
                <h5 className="text-xs font-bold text-slate-100 uppercase tracking-wider font-display">
            ...
            <div className="flex flex-col gap-3">
              {VIDEO_PROMPTS.map((p, idx) => (
                <div key={idx} className="bg-slate-950 p-3.5 rounded-lg border border-slate-800 relative text-left">
                  ...
                  <p className="text-xs text-slate-400 bg-slate-900 p-2.5 rounded border border-slate-800 italic select-all leading-relaxed">
                    "{p.prompt}"
                  </p>
            ...
            <div className="mt-4 p-3 rounded-lg bg-indigo-50/50 border border-indigo-100 text-xs text-slate-400 leading-relaxed">
