...
import { Course, Student, Payment } from './adminTypes';
import { 
  INITIAL_COURSES, 
  INITIAL_STUDENTS, 
  INITIAL_PAYMENTS, 
...
import DateTraveler from './DateTraveler';
import CourseModal from './CourseModal';
import StudentModal from './StudentModal';
import PaymentDetailsModal from './PaymentDetailsModal';
import AlertsModal from './AlertsModal';
import HeaderCustomizeModal from './HeaderCustomizeModal';
...
export default function DocentAdmin({ currentUser: currentUserProp }: { currentUser: string | null }) {
  const { t, lang } = useLanguage();
  // --- AUTH STATE & LOADING ---
  // Internal Firebase user ID — can come from prop (Suite auth) or Firebase auth observer
  const [firebaseUserId, setFirebaseUserId] = useState<string | null>(currentUserProp);
  const currentUser = firebaseUserId;
  const setCurrentUser = setFirebaseUserId;
  ...
  // --- AUTH OBSERVER ---
  useEffect(() => {
    // If the parent already provided a user (from Supabase/Google login), trust it
    if (currentUserProp) {
      setFirebaseUserId(currentUserProp);
      setAuthChecking(false);
      return;
    }
    // Otherwise fall back to Firebase auth for standalone use
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        setFirebaseUserId(user.uid);
      } else {
        // No firebase user — use local demo mode
        setFirebaseUserId('local-user');
        setCourses([]);
        setStudents([]);
        setPayments([]);
      }
  ...
  useEffect(() => {
    ...
    if (currentUser === 'local-user') {
      ...
      const localSubtitle = localStorage.getItem('edugrid_inst_subtitle') || 'Sede Central â€¢ GestiÃ³n de cobros y mora automatizada';
      const localLogo = localStorage.getItem('edugrid_inst_logo') || 'ðŸŽ“';
      ...
      try {
        ...
        if (config || dbCourses.length > 0) {
          ...
          if (config) {
            ...
            setInstitutionSubtitle(config.institutionSubtitle || 'Sede Central â€¢ GestiÃ³n de cobros y mora automatizada');
            setInstitutionLogo(config.institutionLogo || 'ðŸŽ“');
  ...
  const handleDeleteCourse = async (courseId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (window.confirm('Â¿EstÃ¡ seguro de eliminar este cursado? Se desasignarÃ¡n los alumnos y se borrarÃ¡n sus pagos.')) {
  ...
  const handleRemoveStudent = async (studentId: string) => {
    if (window.confirm('Â¿EstÃ¡ seguro de eliminar a este alumno del cursado? Esta acciÃ³n no se puede deshacer y borrarÃ¡ su historial de cuotas.')) {
  ...
  const handleResetAppToDefault = async () => {
    if (window.confirm('Â¿Desea restaurar todos los datos a la configuraciÃ³n demostrativa original? Se perderÃ¡n sus cambios actuales.')) {
      ...
      setInstitutionSubtitle('Sede Central â€¢ GestiÃ³n de cobros y mora automatizada');
      setInstitutionLogo('ðŸŽ“');
      ...
      if (currentUser && currentUser !== 'local-user') {
        ...
        try {
          await syncLocalDataToFirebase(currentUser, INITIAL_COURSES, INITIAL_STUDENTS, INITIAL_PAYMENTS, {
            ...
            institutionSubtitle: 'Sede Central â€¢ GestiÃ³n de cobros y mora automatizada',
            institutionLogo: 'ðŸŽ“',
  ...
  const handleClearAllData = async () => {
    if (window.confirm('âš ï¸ Â¿EstÃ¡ seguro de eliminar TODOS los datos (cursados, alumnos, cobros)? Se iniciarÃ¡ con una planilla vacÃ­a.')) {
      localStorage.removeItem('edugrid_courses');
      localStorage.removeItem('edugrid_students');
      localStorage.removeItem('edugrid_payments');
  ...
  const getCourseTimeLeft = (course: Course, simDate: Date) => {
    ...
    if (monthsRemaining === 0) {
      ...
      if (daysRemaining === 0) {
        ...
        return 'Finaliza maÃ±ana';
      }
      return `Finaliza en ${daysRemaining} dÃ­as`;
    } else if (monthsRemaining === 1) {
      return 'Ãšltimo mes restante';
    } else {
  ...
  // Login screen removed since DocentSuite handles auth
    ...
    <div className="min-h-screen bg-slate-50/40 flex flex-col selection:bg-indigo-100 font-sans antialiased text-slate-800 pb-20">
      ...
      <header className="bg-white border-b border-slate-200/80 py-4.5 px-6 md:px-12 shadow-xs">
        <div className="max-w-7xl mx-auto flex flex-col gap-4.5">
          ...
          {/* Fila Superior: Nombre de la AplicaciÃ³n (Fijo/No Modificable) */}
          ...
          {/* Fila Inferior: InformaciÃ³n de la InstituciÃ³n (Totalmente Personalizable) */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">InstituciÃ³n Activa</span>
                ...
                <h1 className="text-lg font-extrabold text-slate-800 tracking-tight flex items-center gap-2 mt-0.5">
                  ...
                  <button
                    ...
                    onClick={() => setIsHeaderModalOpen(true)}
                    title="Personalizar Nombre y Logo de su InstituciÃ³n"
            ...
            <div className="flex flex-wrap items-center gap-3 self-start md:self-auto">
              ...
              <button
                ...
                title="Borrar todos los datos de prueba y comenzar vacÃ­o"
              ...
              <button
                ...
                title="Click para cambiar logo o nombre de la instituciÃ³n"
              >
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 mt-6 w-full space-y-6">
        ...
        {currentUser === 'local-user' && (
          <div className="bg-amber-50 border border-amber-250 rounded-2xl p-5 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4 animate-fade-in">
            <div className="flex gap-3">
              ...
              <div className="space-y-1">
                <h4 className="text-sm font-black text-amber-950 tracking-tight flex items-center gap-1.5">
                  {lang === 'en' ? 'You are in Local Demo Mode (No Account)' : 'Estás en Modo de Prueba Local (Sin Cuenta)'}
                  <span className="text-[9px] bg-amber-200/60 text-amber-900 font-extrabold px-1.5 py-0.5 rounded uppercase tracking-wider">
                    Firebase {lang === 'en' ? 'Disabled' : 'Desactivado'}
                  </span>
                </h4>
                <p className="text-xs text-amber-800 leading-relaxed max-w-3xl font-medium">
                  {lang === 'en'
                    ? <>The <strong>Email/Password</strong> provider of your Firebase project is disabled, which prevents creating cloud accounts. You can use the app normally (data will be saved in your browser). To enable real sync, go to <strong>Firebase Console &gt; Authentication &gt; Sign-in method &gt; Email/Password &gt; Enable</strong>.</>
                    : <>El proveedor de <strong>Email/Password</strong> de tu proyecto Firebase está desactivado. Puedes usar la app normalmente (los datos se guardarán en tu navegador). Para activar la sincronización real, ve a <strong>Consola de Firebase &gt; Authentication &gt; Sign-in method &gt; Email/Password &gt; Habilitar</strong>.</>
                  }
                </p>
              </div>
            </div>
            ...
            >
              {lang === 'en' ? 'Activation Guide 📋' : 'Ver Guía de Activación 📋'}
            </button>
          </div>
        )}
          ...
          <section className="lg:col-span-1 space-y-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center justify-between">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wider bg-${course.color}-50 text-${course.color}-700`} style={{ backgroundColor: course.color === 'emerald' ? '#ecfdf5' : course.color === 'fuchsia' ? '#fdf4ff' : '#eff6ff', color: course.color === 'emerald' ? '#047857' : course.color === 'fuchsia' ? '#a21caf' : '#1d4ed8' }}>
                            {MONTH_NAMES[course.startMonth]} â€¢ {course.duration} meses
                          </span>
            ...
            <div className="bg-indigo-50/50 border border-indigo-100/60 p-5 rounded-3xl flex gap-3 text-indigo-900 shadow-xs">
              ...
              <div className="text-xs space-y-1.5 leading-relaxed font-medium">
                <span className="font-bold block text-sm text-indigo-955">Â¿CÃ³mo funciona la Mora?</span>
                <p>
                  El sistema detecta atrasos evaluando la <strong>Fecha del simulador</strong> contra el <strong>DÃ­a de Vencimiento</strong> de cada cursado.
                </p>
                <p>
                  Prueba modificando la fecha con los controladores de arriba para ver cÃ³mo los alumnos cambian a estado de alerta automÃ¡ticamente.
                </p>
              </div>
            </div>
          </section>
