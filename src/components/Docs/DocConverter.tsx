...
export default function DocConverter({ onSuccess, applyWatermark = false }: DocConverterProps) {
  ...
  const runConversion = async () => {
    ...
    const logs = isEn 
      ? [
          `Reading file buffer structure (format .${fileItem.extension})...`,
          `Initiating transcoding mapping to .${targetExt}...`,
          `Normalizing character sets and color maps...`,
          `Building abstract syntax tree (AST) of the container...`,
          `Writing format signatures and exporting final bytes...`
        ]
      : [
    ...
    try {
      ...
      addLogMessage(isEn ? 'Transcoding completed successfully!' : '¡Transcodificación completada con éxito!');
      ...
      setFileItem((prev) => prev ? { 
        ...prev, 
        status: 'error', 
        errorMessage: err.message || (isEn ? 'Error during conversion.' : 'Error en la conversión.') 
      } : null);
    }
  };
  ...
  return (
    <div id="document-converter-module" className="backdrop-blur-xl bg-slate-900/50 border border-white/10 rounded-[32px] p-6 relative overflow-hidden transition-all duration-300 hover:border-white/20 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)]">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-display font-semibold text-white tracking-tight flex items-center gap-2">
              {isEn ? 'Universal Converter' : 'Conversor Universal'} <span className="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded-full font-mono font-medium">AUTO</span>
            </h2>
            <p className="text-xs text-slate-300 mt-0.5">{isEn ? 'Convert images, text or tables automatically' : 'Convierte imágenes, textos o tablas automáticamente'}</p>
          </div>
        </div>
      </div>
        ...
        {!fileItem ? (
          ...
          >
            ...
            >
              ...
              />
              <div className="p-4 rounded-full bg-slate-900/50 border border-white/10 text-indigo-400">
                <FileUp className="w-8 h-8" />
              </div>
              <div>
                <p className="text-sm font-medium text-slate-200">{isEn ? 'Upload any file type to convert it' : 'Sube cualquier tipo de archivo para convertirlo'}</p>
                <p className="text-xs text-slate-400 mt-1">{isEn ? 'Supports PDF, PNG, JPG, WEBP, DOCX, XLSX, TXT, CSV, JSON, MD, etc.' : 'Soporta PDF, PNG, JPG, WEBP, DOCX, XLSX, TXT, CSV, JSON, MD, etc.'}</p>
              </div>
            </div>
          </motion.div>
        ) : (
          ...
          >
            ...
            <div className="backdrop-blur-lg bg-slate-900/50 border border-white/10 p-4 rounded-2xl flex items-center justify-between">
              <div className="flex items-center gap-3 overflow-hidden">
                <div className="p-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-indigo-400 font-mono font-bold text-xs shrink-0">
              ...
              {fileItem.status === 'idle' && (
                <button
                  ...
                  title={isEn ? 'Upload another file' : 'Subir otro archivo'}
                >
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <div className="text-xs font-semibold text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
                    ...
                    {isEn ? 'Select Output Format' : 'Selecciona el Formato de Salida'}
                  </div>
                      ...
                      <button
                        ...
                        className={`p-2.5 rounded-xl border text-left transition-all duration-200 flex items-center justify-between group ${
                          targetExt === f.ext
                            ...
                            : 'bg-slate-900/50 border-white/10 text-slate-400 hover:border-white/20 hover:bg-white/10'
                        }`}
                      >
                        <div className="overflow-hidden pr-2">
                          <span className="text-xs font-semibold block group-hover:text-white transition-colors">{f.ext.toUpperCase()}</span>
                          <span className="text-[10px] text-slate-500 block truncate">{isEn ? (f as any).labelEn : f.label}</span>
                        </div>
                        {targetExt === f.ext && (
                ...
                >
                  ...
                  {isEn ? 'Convert File' : 'Convertir Archivo'}
                </button>
              </div>
            )}
                ...
                {logMessages.length > 0 && (
                  <div className="p-3 rounded-xl bg-black/40 border border-white/10 font-mono text-[10px] text-slate-400 space-y-1">
                    <div className="text-slate-500 border-b border-white/5 pb-1 flex justify-between items-center">
                      <span>{isEn ? 'TRANSCODING MODULE' : 'MÓDULO DE TRANSCODIFICACIÓN'}</span>
                      <span className="animate-pulse text-indigo-400">{isEn ? '● LOADING' : '● CARGANDO'}</span>
                    </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-xs text-slate-400">
                    <span className="flex items-center gap-1.5 font-mono">
                      ...
                      {isEn ? 'Processing algorithm...' : 'Procesando algoritmo...'}
                    </span>
            ...
            {fileItem.status === 'completed' && (
              ...
              >
                ...
                <div className="backdrop-blur-md bg-slate-900/50 border border-white/10 p-4 rounded-xl flex items-center justify-center gap-4 font-mono">
                  <div className="text-center">
                    <span className="text-[10px] text-slate-500 block">{isEn ? 'SOURCE' : 'ORIGEN'}</span>
                  ...
                  <div className="text-center">
                    <span className="text-[10px] text-slate-500 block">{isEn ? 'TARGET' : 'DESTINO'}</span>
                    <span className="text-sm font-bold text-indigo-400">.{targetExt.toUpperCase()}</span>
                  </div>
                  <div className="h-6 w-px bg-white/10"></div>
                  <div className="text-center">
                    <span className="text-[10px] text-slate-500 block">{isEn ? 'SIZE' : 'TAMAÑO'}</span>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    {isEn ? 'Download File' : 'Descargar Archivo'}
                  </a>
                  <button
                    onClick={reset}
                    ...
                    title={isEn ? 'Convert another document' : 'Convertir otro documento'}
                  >
            ...
            {fileItem.status === 'error' && (
              <div className="p-4 rounded-2xl bg-red-500/10 border border-red-500/20 space-y-3">
                <div className="flex items-center gap-2 text-red-400 font-medium text-xs">
                  ...
                  {isEn ? 'Error in transcoding' : 'Error en la transcodificación'}
                </div>
