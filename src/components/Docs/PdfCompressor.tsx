...
import { CompressionLevel, FileItem, ProcessingHistoryItem } from './types';
import { useLanguage } from '../../contexts/LanguageContext';
import { compressPdf } from './utils/converter';

interface PdfCompressorProps {
  onSuccess: (item: ProcessingHistoryItem) => void;
  applyWatermark?: boolean;
}

export default function PdfCompressor({ onSuccess, applyWatermark = false }: PdfCompressorProps) {
  ...
  const processFile = (file: File) => {
    if (file.type !== 'application/pdf' && !file.name.endsWith('.pdf')) {
      alert(isEn ? 'Please select PDF files only.' : 'Por favor, selecciona únicamente archivos PDF.');
      return;
    }
  ...
  const runCompression = async () => {
    ...
    const logs = isEn 
      ? [
          'Initiating internal dictionary scan...',
          'Identifying embedded typographic fonts...',
          'Analyzing image stream compression...',
          optimizeDpi ? 'Resampling bitmaps to 150 DPI high definition...' : 'Preserving nominal image resolution...',
          'Re-indexing cross-reference tables (XREF)...',
          preserveMetadata ? 'Retaining EXIF metadata and authorship info...' : 'Discarding redundant metadata...',
          'Generating ultra-optimized packaging stream...',
          'Finalizing document object tree reconstruction...'
        ]
      : [
    ...
    try {
      ...
      addLogMessage(isEn ? 'File successfully optimized by the quantum engine!' : '¡Archivo optimizado exitosamente con el motor cuántico!');
  ...
  return (
    <div id="pdf-compressor-module" className="backdrop-blur-xl bg-slate-900/50 border border-white/10 rounded-[32px] p-6 relative overflow-hidden transition-all duration-300 hover:border-white/20 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)]">
      <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/5 rounded-full blur-3xl pointer-events-none"></div>
      
      {/* Module Title */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="p-3 rounded-2xl bg-slate-900/50 border border-white/10 text-indigo-400">
            <Cpu className="w-6 h-6 animate-pulse" />
          </div>
          <div>
            <h2 className="text-xl font-display font-semibold text-white tracking-tight flex items-center gap-2">
              {isEn ? 'PDF Compressor' : 'Compresor de PDF'} <span className="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded-full font-mono font-medium">ULTRA</span>
            </h2>
            <p className="text-xs text-slate-300 mt-0.5">{isEn ? 'Reduce file size without sacrificing readability' : 'Reduce el peso sin sacrificar legibilidad'}</p>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        {!fileItem ? (
          ...
          >
            ...
            >
              <input
                ...
                id="pdf-file-picker"
              />
              <div className="p-4 rounded-full bg-slate-900/50 border border-white/10 text-indigo-400 group-hover:scale-110 transition-transform duration-300">
                <FileUp className="w-8 h-8" />
              </div>
              <div>
                <p className="text-sm font-medium text-slate-200">{isEn ? 'Drag your PDF file here or click to browse' : 'Arrastra tu archivo PDF aquí o presiona para buscar'}</p>
                <p className="text-xs text-slate-400 mt-1">{isEn ? 'Supports documents of any size' : 'Soporta cualquier tamaño de documento'}</p>
              </div>
            </div>

            {/* Level Selector */}
            <div className="backdrop-blur-lg bg-slate-900/50 border border-white/10 p-5 rounded-2xl space-y-4">
              <div className="flex items-center gap-2 text-xs font-semibold text-slate-300 uppercase tracking-wider">
                <Sliders className="w-4 h-4 text-indigo-400" />
                {isEn ? 'Compression Level' : 'Nivel de Compresión'}
              </div>
              <div className="grid grid-cols-3 gap-2">
                {(['low', 'medium', 'high'] as const).map((lvl) => {
                  const details = {
                    low: { label: isEn ? 'Low' : 'Baja', desc: isEn ? 'Intact quality' : 'Calidad intacta' },
                    medium: { label: isEn ? 'Medium' : 'Media', desc: isEn ? 'Balanced' : 'Equilibrado' },
                    high: { label: isEn ? 'Extreme' : 'Extrema', desc: isEn ? 'Maximum savings' : 'Máximo ahorro' },
                  };
                    ...
                    <button
                      ...
                      className={`p-3 rounded-xl border text-left transition-all duration-300 ${
                        active
                          ...
                          : 'bg-slate-900/50 border-white/10 text-slate-300 hover:border-white/20 hover:bg-white/10'
                      }`}
                    >
              ...
              <div className="border-t border-white/10 pt-4 grid grid-cols-2 gap-4">
                <label className="flex items-center justify-between cursor-pointer group">
                  <div className="space-y-0.5">
                    <span className="text-xs font-medium text-slate-300 group-hover:text-white transition-colors">{isEn ? 'Optimize DPI' : 'Optimizar DPI'}</span>
                    <p className="text-[10px] text-slate-400">{isEn ? 'Resample at 150dpi' : 'Muestreo a 150ppp'}</p>
                  </div>
                  <input
                    type="checkbox"
                    checked={optimizeDpi}
                ...
                <label className="flex items-center justify-between cursor-pointer group">
                  <div className="space-y-0.5">
                    <span className="text-xs font-medium text-slate-300 group-hover:text-white transition-colors">{isEn ? 'Preserve Metadata' : 'Preservar Metadatos'}</span>
                    <p className="text-[10px] text-slate-400">{isEn ? 'Author, date, title' : 'Autor, fecha, título'}</p>
                  </div>
                  <input
                    type="checkbox"
          ...
          >
            {/* File Info Card */}
            <div className="backdrop-blur-lg bg-slate-900/50 border border-white/10 p-4 rounded-2xl flex items-center justify-between">
              <div className="flex items-center gap-3 overflow-hidden">
                <div className="p-3 rounded-xl bg-slate-900/50 border border-white/10 text-rose-400 shrink-0">
                ...
                <div className="overflow-hidden">
                  ...
                  <div className="flex gap-2 text-[10px] text-slate-400 font-mono mt-0.5">
                    ...
                    <span>•</span>
                    <span className="text-indigo-300">{isEn ? 'Compression:' : 'Compresión:'} {level.toUpperCase()}</span>
                  </div>
                </div>
              </div>
              {fileItem.status === 'idle' && (
                <button
                  ...
                  title={isEn ? 'Change file' : 'Cambiar archivo'}
                >
            ...
            {logMessages.length > 0 && (
              <div className="p-3 rounded-xl bg-black/40 border border-white/10 font-mono text-[10px] text-slate-400 space-y-1">
                <div className="text-slate-500 border-b border-white/5 pb-1 flex justify-between items-center">
                  <span>{isEn ? 'PROCESSOR LOGS' : 'REGISTROS DEL PROCESADOR'}</span>
                  <span className="animate-pulse text-indigo-400">{isEn ? '● ACTIVE' : '● ACTIVO'}</span>
                </div>
            ...
            {fileItem.status === 'idle' && (
              ...
              >
                ...
                {isEn ? 'Start Compression' : 'Comenzar Compresión'}
              </button>
            )}
              ...
              <div className="space-y-2 py-2">
                <div className="flex justify-between text-xs text-slate-400">
                  <span className="flex items-center gap-1.5 font-mono">
                    ...
                    {isEn ? 'Optimizing content...' : 'Optimizando contenido...'}
                  </span>
            ...
            {fileItem.status === 'completed' && (
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="backdrop-blur-md bg-slate-900/50 border border-white/10 p-3 rounded-xl">
                    <span className="block text-[9px] text-slate-400 font-medium font-bold">{isEn ? 'ORIGINAL' : 'ORIGINAL'}</span>
                    <span className="text-xs font-bold text-slate-300 font-mono mt-0.5 block">{formatSize(fileItem.size)}</span>
                  </div>
                  <div className="backdrop-blur-md bg-slate-900/50 border border-white/10 p-3 rounded-xl">
                    <span className="block text-[9px] text-emerald-400 font-medium font-bold">{isEn ? 'COMPRESSED' : 'COMPRIMIDO'}</span>
                  ...
                  <div className="bg-emerald-500/10 p-3 rounded-xl border border-emerald-500/20">
                    <span className="block text-[9px] text-emerald-400 font-medium font-bold">{isEn ? 'SAVINGS' : 'AHORRO'}</span>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="p-4 rounded-2xl bg-slate-900/50 hover:bg-white/10 border border-white/10 text-slate-200 hover:text-white transition-colors"
                    title={isEn ? 'Compress another file' : 'Comprimir otro archivo'}
                  >
            ...
            {fileItem.status === 'error' && (
              <div className="p-4 rounded-2xl bg-red-500/10 border border-red-500/20 space-y-3">
                <div className="flex items-center gap-2 text-red-400 font-medium text-xs">
                  ...
                  {isEn ? 'Error detected' : 'Error detectado'}
                </div>
                <p className="text-xs text-slate-400">{fileItem.errorMessage}</p>
                <button
                  onClick={reset}
                  id="reset-on-error-btn"
                  className="w-full py-2 rounded-xl bg-slate-900/50 hover:bg-white/10 text-slate-300 text-xs font-medium border border-white/10 transition-colors"
                >
                  {isEn ? 'Try again' : 'Volver a intentar'}
                </button>
              </div>
            )}
          </motion.div>
