...
async function applyWatermarkToPdfBlob(originalBlob: Blob): Promise<Blob> {
  try {
    const savedBrand = localStorage.getItem('docent_nexus_brand');
    if (!savedBrand) return originalBlob;
    const brand = JSON.parse(savedBrand);
    const brandName = brand.name || 'DOCENT Suite';
    
    // Parse color if exists
    let r = 0.5, g = 0.5, b = 0.5;
    if (brand.colors && brand.colors[0]) {
      const hex = brand.colors[0].replace('#', '');
      if (hex.length === 6) {
...
export async function compressPdf(
  ...
  if (applyWatermark) {
    compressedBlob = await applyWatermarkToPdfBlob(compressedBlob);
  }
  ...
  return {
    blob: compressedBlob,
    size: compressedBlob.size > 0 ? compressedBlob.size : newSize, // use exact size if modified
    name: newName,
  };
}
...
export async function convertDocument(
  ...
  if (targetExt === 'pdf') {
    ...
    if (detectedExt === 'pdf') {
      ...
      if (applyWatermark) finalBlob = await applyWatermarkToPdfBlob(finalBlob);
      return {
        blob: finalBlob,
        size: finalBlob.size,
        name: targetFileName,
    ...
    if (['png', 'jpg', 'jpeg', 'webp'].includes(detectedExt)) {
      try {
        let blob = await convertImageToPdf(file);
        if (applyWatermark) blob = await applyWatermarkToPdfBlob(blob);
        return {
          blob,
          size: blob.size,
          name: targetFileName,
    ...
    if (detectedExt === 'docx') {
      try {
        let blob = await convertDocxToPdf(file);
        if (applyWatermark) blob = await applyWatermarkToPdfBlob(blob);
        return {
          blob,
          size: blob.size,
    ...
    if (detectedExt === 'xlsx') {
      try {
        let blob = await convertXlsxToPdf(file);
        if (applyWatermark) blob = await applyWatermarkToPdfBlob(blob);
        return {
          blob,
          size: blob.size,
    ...
    if (!isBinary) {
      try {
        ...
        let blob = convertTextToPdf(file.name, textContent);
        if (applyWatermark) blob = await applyWatermarkToPdfBlob(blob);
        return {
          blob,
          size: blob.size,
    ...
    let blob = generateMinimalPdfBlob(file.name, sizeStr);
    if (applyWatermark) blob = await applyWatermarkToPdfBlob(blob);
    return {
      blob,
      size: blob.size,
