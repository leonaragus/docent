...
import { SYSTEM_TEMPLATES } from './templatesData';
import { LogoProps, SavedBrand } from './types';
...
export default function App() {
  ...
  const generateBranding = () => {
    ...
    const eventSource = new EventSource(`/api/generate-branding?${params.toString()}`);
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.type === 'progress') {
          ...
          setBrandName(formData.institutionName);
          setBrandSlogan(formData.slogan || 'Excelencia y Compromiso con el Futuro');
          
          if (res.colors && Array.isArray(res.colors)) {
            // Ensure we have 4 colors
            const colors = [...res.colors];
            while (colors.length < 4) colors.push('#1E293B');
            setBrandColors(colors);
          }
          
          // Determine font
          const f = (res.font || '').toLowerCase();
          if (f.includes('playfair') || f.includes('serif')) {
            setBrandFont('classic');
          } else if (f.includes('space') || f.includes('mono') || f.includes('grotesk')) {
            setBrandFont('tech');
          } else {
            setBrandFont('modern');
          }

          // Assign logo symbol automatically
          let sym = 'heraldic_laurel'; // Ultra premium default
          const typeLower = formData.type.toLowerCase();
          const nameLower = formData.institutionName.toLowerCase();
          if (nameLower.includes('raicep') || nameLower.includes('registr') || nameLower.includes('homolog')) {
            sym = 'sovereign_seal';
          } else if (typeLower.includes('educ') || nameLower.includes('escuela') || nameLower.includes('univers')) {
            sym = 'noble_crest';
          } else if (typeLower.includes('salud') || nameLower.includes('méd') || nameLower.includes('salud')) {
            sym = 'leaf';
          } else if (formData.vibe.toLowerCase().includes('tec') || formData.vibe.toLowerCase().includes('inno')) {
            sym = 'corporate_nexus';
          }
          setBrandSymbol(sym);

          setPdfHeaderDescription(res.pdfHeaderDescription || 'Membrete institucional corporativo.');
          setBranding(res);
          setIsLoading(false);
          eventSource.close();
        } else if (data.type === 'error') {
          // If Gemini fails or experiences rate limits, we trigger the intelligent local fallback
          console.warn('Gemini experimentó límites, activando failover local inteligente.');
    ...
    eventSource.onerror = (err) => {
      console.warn('EventSource cerró, activando failover local inteligente.');
      triggerFallback(formData.institutionName, formData.type, formData.vibe, formData.font);
      setIsLoading(false);
      eventSource.close();
    };
...
`;
  ...
  return (
    <div className="min-h-screen bg-[#F6F5FB] text-slate-800 font-sans selection:bg-violet-500 selection:text-white pb-12 relative overflow-hidden">
      ...
      {/* Header */}
      <header className="max-w-7xl mx-auto px-6 pt-10 pb-8 relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-violet-100 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black bg-gradient-to-r from-violet-700 via-indigo-600 to-fuchsia-600 bg-clip-text text-transparent tracking-tight">RAICEP GENERADOR DE MARCA</h1>
            <p className="text-xs text-slate-500 font-semibold tracking-wider uppercase">Registro Argentino de Instituciones y Homologación de Estudios Profesionales</p>
          </div>
        </div>
        {branding && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-violet-200 hover:bg-violet-50/80 text-violet-700 transition text-sm font-semibold shadow-sm"
          >
            <RotateCcw size={16} /> Crear otra marca
          </button>
        )}
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 relative z-10">
        {!branding && !isLoading && (
          <div className="max-w-3xl mx-auto">
            {/* Onboarding steps */}
            <div className="bg-white/90 border border-violet-100 p-8 md:p-10 rounded-3xl shadow-xl shadow-violet-200/40 backdrop-blur-xl relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-violet-500/10 rounded-full blur-3xl pointer-events-none"></div>

              {/* Step indicator */}
              <div className="flex items-center gap-4 mb-8">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${step === 1 ? 'bg-violet-500 text-white shadow-lg shadow-violet-500/30 ring-4 ring-violet-500/20' : 'bg-emerald-500 text-white'}`}>
                  {step > 1 ? <Check size={16} /> : '1'}
                </div>
                <div className="h-0.5 bg-violet-100 flex-1"></div>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${step === 2 ? 'bg-violet-500 text-white shadow-lg shadow-violet-500/30 ring-4 ring-violet-500/20' : 'bg-violet-100 text-violet-500'}`}>
                  2
                </div>
              </div>
              ...
              {step === 1 ? (
                <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                  <div>
                    <h2 className="text-3xl font-extrabold text-slate-800 tracking-tight">Comencemos tu Marca</h2>
                    <p className="text-slate-500 text-sm mt-1">Introduce el nombre oficial de la institución que representas.</p>
                  </div>

                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Nombre de la Institución</label>
                      <input
                        type="text"
                        value={formData.institutionName}
                        onChange={(e) => setFormData({ ...formData, institutionName: e.target.value })}
                        className="w-full bg-slate-50/80 border-2 border-violet-100 rounded-2xl px-5 py-4 text-slate-800 focus:outline-none focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 transition text-lg font-medium placeholder:text-slate-450"
                        placeholder="ej. Colegio Máximo San Martín, Universidad Tecnológica..."
                      />
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Eslogan o Lema Institucional (Opcional)</label>
                      <input
                        type="text"
                        value={formData.slogan}
                        onChange={(e) => setFormData({ ...formData, slogan: e.target.value })}
                        className="w-full bg-slate-50/80 border-2 border-violet-100 rounded-2xl px-5 py-3.5 text-slate-800 focus:outline-none focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 transition text-sm placeholder:text-slate-450"
                        placeholder="ej. Formando Líderes para el Mañana"
                      />
                    </div>
                  </div>
                  ...
                  >
                    Definir Estilo & Preferencias <ChevronRight size={18} />
                  </button>
                </motion.div>
              ) : (
                <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                  <div>
                    <h2 className="text-3xl font-extrabold text-slate-800 tracking-tight">Estilo y Categoría</h2>
                    <p className="text-slate-500 text-sm mt-1">Define el sector y el tono visual que se adaptarán a tu membrete.</p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Tipo de Institución</label>
                      <select
                        value={formData.type}
                        onChange={(e) => setFormData({ ...formData, type: e.target.value })}
                        className="w-full bg-slate-50 border-2 border-violet-100 rounded-2xl px-4 py-3.5 text-slate-800 focus:outline-none focus:border-violet-500 transition font-medium"
                      >
                        <option value="Educativo">Educativo (Colegio, Universidad)</option>
                        <option value="Corporativo">Corporativo (Empresa, Oficina)</option>
                        <option value="Salud">Salud (Clínica, Sanatorio)</option>
                        <option value="Tecnológico">Tecnológico (Laboratorio, Desarrollo)</option>
                        <option value="Creativo">Creativo (Agencia, Fundación)</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Estilo Visual (Vibe)</label>
                      <select
                        value={formData.vibe}
                        onChange={(e) => setFormData({ ...formData, vibe: e.target.value })}
                        className="w-full bg-slate-50 border-2 border-violet-100 rounded-2xl px-4 py-3.5 text-slate-800 focus:outline-none focus:border-violet-500 transition font-medium"
                      >
                        <option value="Solemne / Académico">Solemne / Académico</option>
                        <option value="Moderna / Tecnológica">Moderna / Tecnológica</option>
                        <option value="Mínimal Organico">Mínimal Orgánico</option>
                        <option value="Vibrante / Creativa">Vibrante / Creativa</option>
                        <option value="Clásica Corporativa">Clásica Corporativa</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Tipografía Preferida</label>
                      <select
                        value={formData.font}
                        onChange={(e) => setFormData({ ...formData, font: e.target.value })}
                        className="w-full bg-slate-50 border-2 border-violet-100 rounded-2xl px-4 py-3.5 text-slate-800 focus:outline-none focus:border-violet-500 transition font-medium"
                      >
                        <option value="Moderna">Moderna (Outfit / Sans)</option>
                        <option value="Clásica">Clásica (Playfair / Serif)</option>
                        <option value="Técnica">Técnica (Space / Mono)</option>
                      </select>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Teléfono</label>
                        <input
                          type="text"
                          value={formData.phone}
                          onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
                          className="w-full bg-slate-50 border-2 border-violet-100 rounded-xl px-3 py-2.5 text-xs text-slate-800 focus:outline-none focus:border-violet-500 transition"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Web / Dominio</label>
                        <input
                          type="text"
                  ...
                  <div className="flex gap-4 pt-4">
                    <button
                      onClick={() => setStep(1)}
                      className="w-1/3 bg-slate-100 border border-violet-200 hover:bg-slate-200/80 text-violet-700 py-4 rounded-2xl font-bold transition flex items-center justify-center gap-2"
                    >
                    ...
                    >
                      Generar Identidad de Marca <Sparkles size={18} />
                    </button>
                  </div>
                </motion.div>
              )}
        ...
        {isLoading && (
          <div className="max-w-2xl mx-auto py-16">
            <div className="bg-white/95 border border-violet-100 rounded-3xl p-10 flex flex-col items-center justify-center text-center shadow-xl shadow-violet-200/50 backdrop-blur-xl relative overflow-hidden">
              ...
              <h3 className="text-xl font-bold text-slate-800 mb-2">Diseñando Identidad Institucional</h3>
              <p className="text-violet-600 text-sm font-semibold bg-violet-50 border border-violet-100 px-4 py-2 rounded-full animate-pulse">
                {progressMessage || 'Creando composiciones vectoriales...'}
              </p>
              
              <div className="w-full max-w-sm bg-slate-100 h-2.5 rounded-full mt-6 overflow-hidden relative border border-violet-200/50">
              ...
              <p className="text-xs text-slate-500 mt-4 leading-relaxed max-w-xs">
                Analizando tipografías y coordinando colores corporativos adecuados para documentos oficiales y banners.
              </p>
            </div>
          </div>
        )}
        ...
        {branding && !isLoading && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-white/95 border border-violet-100 rounded-3xl shadow-xl shadow-violet-200/30 overflow-hidden backdrop-blur-md">
                
                {/* Customizer Tabs */}
                <div className="flex bg-slate-50 border-b border-violet-100 p-1.5 gap-1">
                  <button
                    onClick={() => setEditorTab('colors')}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-3 px-2 rounded-xl text-xs font-bold transition-all ${editorTab === 'colors' ? 'bg-gradient-to-r from-violet-500 to-fuchsia-600 text-white shadow-lg' : 'text-slate-500 hover:bg-violet-50 hover:text-violet-700'}`}
                  >
                    <Palette size={14} /> Colores
                  </button>
                  <button
                    onClick={() => setEditorTab('logo')}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-3 px-2 rounded-xl text-xs font-bold transition-all ${editorTab === 'logo' ? 'bg-gradient-to-r from-violet-500 to-fuchsia-600 text-white shadow-lg' : 'text-slate-500 hover:bg-violet-50 hover:text-violet-700'}`}
                  >
                    <Sliders size={14} /> Logo
                  </button>
                  <button
                    onClick={() => setEditorTab('text')}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-3 px-2 rounded-xl text-xs font-bold transition-all ${editorTab === 'text' ? 'bg-gradient-to-r from-violet-500 to-fuchsia-600 text-white shadow-lg' : 'text-slate-500 hover:bg-violet-50 hover:text-violet-700'}`}
                  >
                    <Type size={14} /> Textos
                  </button>
                  <button
                    onClick={() => setEditorTab('pdf')}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-3 px-2 rounded-xl text-xs font-bold transition-all ${editorTab === 'pdf' ? 'bg-gradient-to-r from-violet-500 to-fuchsia-600 text-white shadow-lg' : 'text-slate-500 hover:bg-violet-50 hover:text-violet-700'}`}
                  >
                ...
                <div className="p-6 space-y-6">
                  ...
                  {editorTab === 'colors' && (
                    <div className="space-y-5">
                      <div>
                        <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-1">Paleta de Colores</h4>
                        <p className="text-xs text-slate-500">Modifica los colores corporativos exactos para tu membrete y logo.</p>
                      </div>

                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-3">
                          <div className="bg-slate-50 border border-violet-100 p-3 rounded-2xl flex items-center justify-between gap-2">
                            <div>
                              <span className="text-[10px] font-bold text-slate-500 block uppercase">Primario</span>
                              <span className="text-xs font-mono font-bold text-slate-750">{brandColors[0]}</span>
                            </div>
                            <input
                              type="color"
                              value={brandColors[0]}
                              ...
                              className="w-10 h-10 rounded-xl cursor-pointer border-2 border-violet-200 bg-transparent overflow-hidden"
                            />
                          </div>

                          <div className="bg-slate-50 border border-violet-100 p-3 rounded-2xl flex items-center justify-between gap-2">
                            <div>
                              <span className="text-[10px] font-bold text-slate-500 block uppercase">Secundario</span>
                              <span className="text-xs font-mono font-bold text-slate-750">{brandColors[1]}</span>
                            </div>
                            <input
                              type="color"
                              value={brandColors[1]}
                          ...
                          <div className="bg-slate-50 border border-violet-100 p-3 rounded-2xl flex items-center justify-between gap-2">
                            <div>
                              <span className="text-[10px] font-bold text-slate-500 block uppercase">Acento</span>
                              <span className="text-xs font-mono font-bold text-slate-750">{brandColors[2]}</span>
                            </div>
                            <input
                              type="color"
                              value={brandColors[2]}
                          ...
                          <div className="bg-slate-50 border border-violet-100 p-3 rounded-2xl flex items-center justify-between gap-2">
                            <div>
                              <span className="text-[10px] font-bold text-slate-500 block uppercase">Oscuro</span>
                              <span className="text-xs font-mono font-bold text-slate-750">{brandColors[3] || '#1E293B'}</span>
                            </div>
                            <input
                              type="color"
                        ...
                        <div>
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Ajustar Tipografía General</label>
                          <div className="grid grid-cols-2 gap-2">
                            <button
                              onClick={() => setBrandFont('modern')}
                              className={`py-2 px-3 rounded-xl text-xs font-bold border transition ${brandFont === 'modern' ? 'bg-violet-600 border-violet-600 text-white shadow-md shadow-violet-500/10' : 'bg-slate-50 border-violet-100 hover:bg-slate-100 text-slate-600'}`}
                            >
                              Sans-Serif (Outfit)
                            </button>
                            <button
                              onClick={() => setBrandFont('classic')}
                              className={`py-2 px-3 rounded-xl text-xs font-bold border transition ${brandFont === 'classic' ? 'bg-violet-600 border-violet-600 text-white shadow-md shadow-violet-500/10' : 'bg-slate-50 border-violet-100 hover:bg-slate-100 text-slate-600'}`}
                            >
                              Serif Tradicional
                            </button>
                            <button
                              onClick={() => setBrandFont('tech')}
                              className={`py-2 px-3 rounded-xl text-xs font-bold border transition ${brandFont === 'tech' ? 'bg-violet-600 border-violet-600 text-white shadow-md shadow-violet-500/10' : 'bg-slate-50 border-violet-100 hover:bg-slate-100 text-slate-600'}`}
                            >
                              Sleek Display / Mono
                            </button>
                            <button
                              onClick={() => setBrandFont('elegant')}
                              className={`py-2 px-3 rounded-xl text-xs font-bold border transition ${brandFont === 'elegant' ? 'bg-violet-600 border-violet-600 text-white shadow-md shadow-violet-500/10' : 'bg-slate-50 border-violet-100 hover:bg-slate-100 text-slate-600'}`}
                            >
                              Elegant Lora
                            </button>
                          </div>
                  ...
                  {editorTab === 'logo' && (
                    <div className="space-y-5">
                      <div>
                        <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-1">Estructura del Logotipo</h4>
                        <p className="text-xs text-slate-500">Selecciona el símbolo vectorizado y la disposición del texto.</p>
                      </div>

                      <div className="space-y-4">
                        <div>
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2.5">Símbolo Iconográfico (25 Variantes Vectoriales)</label>
                          <div className="grid grid-cols-4 gap-2 max-h-64 overflow-y-auto pr-1.5 custom-scrollbar">
                            {[
                              ...
                              <button
                                ...
                                className={`p-2 py-3 rounded-xl border transition flex flex-col items-center justify-between min-h-[64px] gap-1 ${brandSymbol === sym.id ? 'bg-violet-100 border-violet-400 text-violet-700 shadow-sm font-bold' : 'bg-slate-50 border-violet-100/50 hover:border-violet-200 text-slate-650'}`}
                              >
                        ...
                        <div>
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Estructura del Logo</label>
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              ...
                              <button
                                ...
                                className={`py-2 px-3 rounded-xl text-xs font-bold border transition ${brandLayout === layout.id ? 'bg-violet-600 border-violet-600 text-white shadow-md shadow-violet-500/10' : 'bg-slate-50 border-violet-100 hover:bg-slate-100 text-slate-600'}`}
                              >
                                {layout.label}
                              </button>
                            ))}
                        ...
                        <div>
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Acabado y Personalidad del Isotipo (Muy Moderno)</label>
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              ...
                              <button
                                ...
                                className={`py-2 px-2.5 rounded-xl text-xs font-bold border transition ${brandStyle === styleOpt.id ? 'bg-gradient-to-r from-violet-600 to-fuchsia-600 border-transparent text-white shadow-md' : 'bg-slate-50 border-violet-100 hover:bg-slate-100 text-slate-600'}`}
                              >
                                {styleOpt.label}
                              </button>
                            ))}
                  ...
                  {editorTab === 'text' && (
                    <div className="space-y-4">
                      <div>
                        <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-1">Información Textual</h4>
                        <p className="text-xs text-slate-500">Edita los textos de contacto e institucionales de manera dinámica.</p>
                      </div>
                      ...
                      <div className="space-y-3.5">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-slate-50 border border-violet-100 rounded-xl px-4 py-2.5 text-xs text-slate-800 focus:outline-none focus:border-violet-500"
                          />
                        </div>

                        <div>
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Eslogan / Lema</label>
                          <input
                            type="text"
                            value={brandSlogan}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Ubicación</label>
                            <input
                              type="text"
                              value={formData.location}
                              onChange={(e) => setFormData({ ...formData, location: e.target.value })}
                              className="w-full bg-slate-50 border border-violet-100 rounded-xl px-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-violet-500"
                            />
                          </div>
                          <div>
                  ...
                  {editorTab === 'pdf' && (
                    <div className="space-y-4">
                      <div>
                        <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-1">Formato de Membrete PDF</h4>
                        <p className="text-xs text-slate-500">Personaliza la maqueta del documento real para impresión.</p>
                      </div>

                      <div className="space-y-4">
                        <div>
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Estilo de Encabezado</label>
                          <div className="grid grid-cols-3 gap-2">
                            {[
                              ...
                              <button
                                ...
                                className={`py-2 px-1 rounded-xl text-[11px] font-bold border transition ${pdfHeaderStyle === style.id ? 'bg-violet-600 border-violet-600 text-white shadow-md' : 'bg-slate-50 border-violet-100 text-slate-600 hover:bg-slate-100'}`}
                              >
                                {style.label}
                              </button>
                            ))}
                        ...
                        <div>
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Título del Documento</label>
                          <input
                            type="text"
                            value={pdfTitle}
                            onChange={(e) => setPdfTitle(e.target.value)}
                            className="w-full bg-slate-50 border border-violet-100 rounded-xl px-4 py-2 text-xs text-slate-800 focus:outline-none focus:border-violet-500"
                          />
                        </div>

                        <div>
                          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Cuerpo del Documento</label>
                          <textarea
                            value={pdfBody}
                            onChange={(e) => setPdfBody(e.target.value)}
                            rows={4}
                            className="w-full bg-slate-50 border border-violet-100 rounded-xl px-4 py-2 text-xs text-slate-800 focus:outline-none resize-none leading-relaxed focus:border-violet-500"
                          />
                        </div>
                      </div>
                    </div>
                ...
                {/* Toolbar actions */}
                <div className="bg-slate-50 p-4 border-t border-violet-100 flex flex-wrap gap-2">
                  <button
                  ...
                  <button
                    ...
                    className="flex-1 min-w-[120px] bg-violet-50 border border-violet-100 text-violet-700 py-2.5 px-3 rounded-xl text-xs font-bold hover:bg-violet-100/70 transition flex items-center justify-center gap-1.5"
                  >
              ...
              <div className="bg-white/60 border border-violet-100/80 rounded-3xl p-5 text-xs text-slate-600 space-y-3 shadow-sm shadow-violet-100/20">
                <h5 className="font-bold text-slate-800 flex items-center gap-1.5"><HelpCircle size={14} /> Guía Práctica</h5>
                ...
                <div className="bg-violet-50/50 p-3 rounded-xl border border-violet-100/80">
                  <span className="font-bold text-slate-800 block mb-1">Consejo Pro:</span>
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="flex bg-slate-50 p-1.5 rounded-2xl border border-violet-100 gap-1 flex-wrap md:flex-nowrap">
                <button
                  onClick={() => setActiveTab('visual')}
                  className={`flex-1 min-w-[100px] flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'visual' ? 'bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white shadow-md' : 'text-slate-500 hover:text-violet-700 hover:bg-violet-50/50'}`}
                >
                  <Eye size={13} /> Manual de Identidad
                </button>
                <button
                  onClick={() => setActiveTab('pdf')}
                  className={`flex-1 min-w-[100px] flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'pdf' ? 'bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white shadow-md' : 'text-slate-500 hover:text-violet-700 hover:bg-violet-50/50'}`}
                >
                  <FileText size={13} /> Membrete PDF
                </button>
                <button
                  onClick={() => setActiveTab('variants')}
                  className={`flex-1 min-w-[100px] flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'variants' ? 'bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white shadow-md' : 'text-slate-500 hover:text-violet-700 hover:bg-violet-50/50'}`}
                >
                  <Layers size={13} /> Variantes
                </button>
                <button
                  onClick={() => setActiveTab('mockups')}
                  className={`flex-1 min-w-[100px] flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'mockups' ? 'bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white shadow-md' : 'text-slate-500 hover:text-violet-700 hover:bg-violet-50/50'}`}
                >
                  <Tv size={13} /> Maquetas
                </button>
                <button
                  onClick={() => setActiveTab('guidelines')}
                  className={`flex-1 min-w-[100px] flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'guidelines' ? 'bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white shadow-md' : 'text-slate-500 hover:text-violet-700 hover:bg-violet-50/50'}`}
                >
                  <ShieldCheck size={13} /> Guías Oficiales
                </button>
                <button
                  onClick={() => setActiveTab('database')}
                  className={`flex-1 min-w-[100px] flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'database' ? 'bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white shadow-md' : 'text-slate-500 hover:text-violet-700 hover:bg-violet-50/50'}`}
                >
              ...
              <div className="bg-white/95 border border-violet-100 rounded-3xl p-6 min-h-[500px] shadow-xl shadow-violet-100/10">
                ...
                {activeTab === 'visual' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                    ...
                    <div className="bg-slate-50/55 border border-violet-100/50 rounded-2xl p-8 flex flex-col items-center justify-center relative overflow-hidden">
                      <div className="absolute top-2 left-2 text-[10px] font-bold text-violet-600 bg-violet-100 px-2 py-1 rounded">LOGOTIPO OFICIAL</div>
                      
                      {/* Flex/Grid Logo layout depending on user selection */}
                      <div className="py-12 flex flex-col items-center justify-center">
                        <div className={`flex ${brandLayout === 'vertical' ? 'flex-col' : brandLayout === 'badge' ? 'flex-col border-4 border-dashed border-violet-500/20 rounded-full p-10 aspect-square justify-center' : 'flex-row'} items-center gap-5 justify-center text-center`}>
                          ...
                          {brandLayout !== 'icon' && (
                            <div className={`${brandLayout === 'vertical' || brandLayout === 'badge' ? 'text-center' : 'text-left'}`}>
                              ...
                              {brandSlogan && (
                                <p className="text-sm font-bold text-slate-500 mt-1.5 uppercase tracking-widest text-[10px]">
                                  {brandSlogan}
                                </p>
                              )}
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="bg-slate-50 border border-violet-100/60 rounded-2xl p-5 flex flex-col gap-4 shadow-sm">
                        <span className="text-[10px] font-bold text-slate-500 uppercase">Aplicaciones Monocromáticas</span>
                        <div className="grid grid-cols-2 gap-3">
                          {/* Positive */}
                          <div className="bg-white p-4 rounded-xl flex items-center justify-center border border-slate-100 shadow-sm">
                      ...
                      <div className="bg-slate-50 border border-violet-100/60 rounded-2xl p-5 flex flex-col gap-3 shadow-sm">
                        <span className="text-[10px] font-bold text-slate-500 uppercase">Colores y Códigos Hex</span>
                        <div className="grid grid-cols-4 gap-2">
                          {brandColors.map((color, i) => (
                            ...
                            >
                              <div className="w-full h-11 rounded-xl shadow-inner border border-violet-100 relative overflow-hidden transition group-hover:scale-105" style={{ backgroundColor: color }}>
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition">
                                  <Copy size={12} className="text-white" />
                                </div>
                              </div>
                              <span className="text-[9px] font-mono font-bold text-slate-650">{copiedColor === color ? '¡Ok!' : color}</span>
                            </button>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="bg-violet-50/50 border border-violet-100 rounded-2xl p-5 space-y-3 shadow-sm">
                      <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2"><FileText size={16} className="text-violet-600" /> Manual y Concepto del Diseño</h4>
                ...
                {activeTab === 'pdf' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                    ...
                    <div
                      id="printable-area"
                      ref={printAreaRef}
                      className="bg-white text-slate-900 rounded-2xl shadow-2xl p-8 md:p-12 relative overflow-hidden mx-auto max-w-[620px] min-h-[750px] border border-slate-200"
                    >
                        ...
                        <div className="border-b-2 pb-6 mb-10 flex items-start justify-between gap-4" style={{ borderColor: brandColors[0] }}>
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              {brandSlogan && <p className="text-[8px] uppercase tracking-wider text-slate-500 font-semibold">{brandSlogan}</p>}
                            </div>
                          </div>

                          <div className="text-right text-[9px] text-slate-500 leading-relaxed font-mono">
                      ...
                      {pdfHeaderStyle === 'traditional' && (
                        <div className="flex flex-col items-center text-center pb-6 mb-10 border-b border-double border-slate-300">
                          ...
                          {brandSlogan && (
                            <p className="text-[10px] italic text-slate-500 font-medium font-serif mt-1">
                              « {brandSlogan} »
                            </p>
                          )}
                ...
                {activeTab === 'variants' && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                    <div>
                      <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-1">Explorar Alternativas de Estilo</h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {COLOR_PRESETS.map((preset) => (
                        <button
                          ...
                          className="text-left bg-slate-50 border border-violet-100/80 hover:border-violet-350 hover:bg-violet-50/20 p-5 rounded-2xl flex flex-col gap-3 transition group shadow-sm"
                        >
                          <div className="flex items-center justify-between w-full">
                            <span className="text-xs font-bold text-slate-800 group-hover:text-violet-700 transition">{preset.name}</span>
                            <span className="text-[9px] uppercase tracking-wider bg-violet-100/80 text-violet-700 px-2.5 py-0.5 rounded-full font-bold">
                              {preset.vibe}
                            </span>
                          </div>
                          ...
                          <div className="flex gap-2.5 my-1">
                            {preset.colors.map((color) => (
                              <div key={color} className="w-7 h-7 rounded-lg border border-violet-100" style={{ backgroundColor: color }} />
                            ))}
                          </div>

                          <span className="text-[10px] text-slate-500 leading-relaxed">
                            Cargar tipografía <strong className="text-slate-800 capitalize">{preset.font}</strong> y paleta de tonos corporativos.
                          </span>
                        </button>
                      ))}
                    </div>
                ...
                {activeTab === 'mockups' && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                    <div>
                      <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-1">Simulador de Maquetas Físicas</h4>
                      <p className="text-xs text-slate-500">
                        Visualiza cómo lucirá la identidad institucional aplicada sobre soportes táctiles de alto valor y señalética premium.
                      </p>
                    </div>
                    ...
                    <div className="flex bg-[#F1EFEA] p-1 rounded-xl border border-stone-200 gap-1">
                      {[
                        { id: 'plaque', label: 'Placa de Vidrio' },
                        { id: 'badge', label: 'Credencial ID' }
                      ].map((model) => (
                        <button
                          key={model.id}
                    ...
                    {/* Preview canvas */}
                    <div className="relative bg-slate-100 border border-slate-200 rounded-2xl p-8 min-h-[420px] flex items-center justify-center overflow-hidden shadow-inner">
                      ...
                      {mockupModel === 'plaque' && (
                        <motion.div
                          ...
                          className="relative w-full max-w-[460px] bg-white/80 border border-violet-100/60 rounded-2xl shadow-xl p-8 backdrop-blur-md flex flex-col items-center justify-between text-center min-h-[280px]"
                        >
                          ...
                          <div className="my-auto space-y-4 relative z-10">
                            <div className="flex justify-center">
                              <div className="bg-slate-50 p-4 rounded-full border border-violet-100/50 shadow-sm">
                                <LogoSymbol symbol={brandSymbol} primary={brandColors[0]} secondary={brandColors[2]} size={64} style={brandStyle} />
                              </div>
                            </div>
                            <div>
                              <h3 className={`text-xl font-bold tracking-tight text-slate-800 ${getFontFamilyClass(brandFont)} uppercase`}>
                            ...
                            <p className="text-[10px] text-stone-600 tracking-wider font-semibold">SEDE CORPORATIVA PRINCIPAL</p>
                          </div>
                        </motion.div>
                      )}
                      ...
                      {mockupModel === 'badge' && (
                        <motion.div
                          initial={{ scale: 0.95, opacity: 0 }}
                          animate={{ scale: 1, opacity: 1 }}
                          className="relative w-[240px] bg-white border border-violet-100 rounded-2xl shadow-xl p-5 text-slate-800 flex flex-col justify-between min-h-[360px] overflow-hidden"
                        >
                          {/* Lanyard Strap Slot */}
                          <div className="absolute top-2 left-1/2 transform -translate-x-1/2 w-10 h-3.5 bg-slate-100 border border-slate-200 rounded-full flex items-center justify-center">
                          ...
                          <div className="pt-10 flex flex-col items-center text-center space-y-4 relative z-10">
                            <div className="bg-slate-50 p-2 rounded-full border border-violet-100 shadow-sm">
                            ...
                            <div>
                              <p className="text-[9px] font-bold tracking-wider text-violet-700">DIRECCIÓN GENERAL</p>
                ...
                {activeTab === 'guidelines' && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                    ...
                    <div className="space-y-4 text-slate-800">
                      <div>
                        <h4 className="text-xs font-bold text-slate-700 uppercase tracking-widest border-b border-violet-100 pb-2">
                          Calculadora de Accesibilidad WCAG (Contraste de Color)
                        </h4>
                        <p className="text-xs text-slate-600 mt-1">
                          El estándar internacional <strong>WCAG 2.1</strong> exige un contraste mínimo para asegurar que estudiantes, pacientes o ciudadanos con dificultades visuales lean correctamente tus comunicados institucionales.
                        </p>
                      </div>

                      {/* Live calculator widget */}
                      <div className="bg-slate-50 border border-violet-100/80 rounded-2xl p-5 grid grid-cols-1 md:grid-cols-3 gap-4 items-center shadow-sm">
                        <div className="space-y-1">
                          <span className="text-[10px] text-slate-500 uppercase font-bold block">Fórmula de Luminancia</span>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-slate-500">vs Fondo Blanco</span>
                          </div>
                        </div>
                            ...
                            return (
                              <>
                                <div className={`p-2 rounded-xl border ${passesNormalAA ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-rose-50 border-rose-200 text-rose-700'}`}>
                                  <p className="font-medium text-slate-600 block uppercase text-[8px] mb-0.5">Texto Normal AA</p>
                                ...
                                <div className={`p-2 rounded-xl border ${passesLargeAA ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-rose-50 border-rose-200 text-rose-700'}`}>
                                  <p className="font-medium text-slate-600 block uppercase text-[8px] mb-0.5">Texto Grande AA</p>
                                ...
                                <div className={`p-2 rounded-xl border ${passesNormalAAA ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-rose-50 border-rose-200 text-rose-700'}`}>
                                  <p className="font-medium text-slate-600 block uppercase text-[8px] mb-0.5">Texto Normal AAA</p>
                    ...
                    <div className="space-y-4">
                      <div>
                        <h5 className="text-xs font-bold text-slate-700 uppercase tracking-widest border-b border-violet-100 pb-2">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
                        ...
                        <div className="md:col-span-5 bg-slate-50 border border-violet-100/80 p-5 rounded-2xl space-y-4 shadow-sm">
                          <div className="space-y-1">
                            <label className="text-[10px] uppercase font-bold text-slate-550 flex justify-between">
                              <span>Densidad de Ondas</span>
                              <span className="font-mono text-slate-800">{guillocheDensity}</span>
                            </label>
                            <input
                              type="range"
                              min="10"
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] uppercase font-bold text-slate-550 flex justify-between">
                              ...
                              <span className="font-mono text-slate-800">{guillocheStroke}px</span>
                            </label>
                            <input
                              type="range"
                              min="0.1"
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] uppercase font-bold text-slate-550 flex justify-between">
                              ...
                              <span className="font-mono text-slate-800">{(guillocheOpacity * 100).toFixed(0)}%</span>
                            </label>
                            <input
                              type="range"
                              min="0.05"
                        ...
                        <div className="md:col-span-7 bg-white border border-violet-100 rounded-2xl h-52 relative flex items-center justify-center overflow-hidden shadow-sm">
                          <span className="absolute top-2 left-2 text-[8px] font-mono bg-slate-50 border border-slate-100 text-slate-500 px-1.5 py-0.5 rounded uppercase">SIMULACIÓN PREVENTIVA</span>
                          ...
                          <div className="relative z-10 text-center space-y-1 select-none pointer-events-none">
                            ...
                            <p className="text-[9px] font-mono text-slate-500">CÓDIGO DE INTEGRIDAD REFORZADA</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
                    <div>
                      <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wider mb-1 flex items-center gap-2">
                        <Database className="text-violet-600 animate-pulse" size={16} /> Biblioteca y Base de Datos de Identidades Institucionales
                      </h4>
                      <p className="text-xs text-slate-650">
                        Carga al instante combinaciones premium de la base de datos de presets pre-diseñados offline (diseñados para no depender de IA), o gestiona tus marcas personalizadas guardadas.
                      </p>
                    </div>

                    {/* Section A: Saved Custom Brands */}
                    <div className="space-y-4">
                      <h5 className="text-xs font-bold text-slate-700 uppercase tracking-widest border-b border-violet-100 pb-2 flex items-center justify-between">
                        <span>Tus Diseños Personalizados Guardados ({savedBrands.length})</span>
                        {savedBrands.length > 0 && (
                          <span className="text-[9px] text-slate-500 font-mono">Buzón de Almacenamiento Local</span>
                        )}
                      </h5>

                      {savedBrands.length === 0 ? (
                        <div className="bg-slate-50 border border-dashed border-violet-150 rounded-2xl p-8 text-center text-slate-600 text-xs">
                          <p className="mb-2">Aún no has guardado ningún diseño personalizado.</p>
                          <p className="text-[11px] text-slate-500">Modifica la marca en el panel izquierdo y haz clic en "Guardar Marca" para almacenarla permanentemente aquí.</p>
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {savedBrands.map((brand) => (
                            <div
                              ...
                              className="text-left bg-slate-50 border border-violet-150/60 hover:border-violet-400 hover:bg-violet-50/20 p-5 rounded-2xl flex flex-col gap-3 transition group cursor-pointer relative shadow-sm"
                            >
                              <button
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div className="max-w-[70%]">
                                  <h6 className="text-xs font-extrabold text-slate-800 truncate">{brand.brandName}</h6>
                                  {brand.brandSlogan && <p className="text-[8px] text-slate-600 truncate uppercase">{brand.brandSlogan}</p>}
                                </div>
                              </div>

                              <div className="flex gap-1.5 my-1">
                                {brand.brandColors.map((color) => (
                                  <div key={color} className="w-5 h-5 rounded border border-violet-100" style={{ backgroundColor: color }} />
                                ))}
                              </div>

                              <div className="flex items-center justify-between mt-1 pt-2 border-t border-violet-100 text-[9px] text-slate-500 font-mono">
                                <span>Tipografía: {brand.brandFont}</span>
                                <span>Guardado: {brand.savedAt}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="space-y-4 pt-4">
                      <h5 className="text-xs font-bold text-slate-700 uppercase tracking-widest border-b border-violet-100 pb-2">
                        Presets del Sistema Pre-Diseñados Offline ({SYSTEM_TEMPLATES.length})
                      </h5>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {SYSTEM_TEMPLATES.map((brand, idx) => (
                          <div
                            ...
                            className="text-left bg-slate-50 border border-violet-100/80 hover:border-violet-400 hover:bg-violet-50/20 p-5 rounded-2xl flex flex-col gap-3 transition group cursor-pointer shadow-sm"
                          >
                            <div className="flex items-center justify-between w-full">
                              <div className="flex items-center gap-2">
                                <LogoSymbol symbol={brand.symbol} primary={brand.colors[0]} secondary={brand.colors[2] || brand.colors[1]} size={32} />
                                <div className="max-w-[70%]">
                                  <h6 className="text-xs font-extrabold text-slate-800 group-hover:text-violet-700 transition truncate">{brand.institutionName}</h6>
                                  {brand.slogan && <p className="text-[8px] text-slate-500 truncate uppercase">{brand.slogan}</p>}
                                </div>
                              </div>
                              <span className="text-[8px] uppercase tracking-wider bg-violet-100/80 text-violet-700 px-2.5 py-0.5 rounded-full font-bold">
                            ...
                            <p className="text-[10px] text-slate-600 leading-relaxed truncate">
                              {brand.description || "Estilo premium listo para cargar."}
                            </p>

                            <div className="flex items-center justify-between mt-1 pt-2 border-t border-violet-100 text-[9px] text-slate-500 font-mono">
                              <span>Tipografía: {brand.font}</span>
                              <span className="text-violet-650 font-semibold uppercase text-[8px]">Cargar Preset</span>
                            </div>
                          </div>
                        ))}
                      </div>
      ...
      <footer className="max-w-7xl mx-auto px-6 mt-16 text-center text-xs text-slate-550 relative z-10 space-y-2">
        <p className="font-semibold uppercase tracking-wider text-slate-600">RAICEP — Generador de Marca Institucional</p>
        <p>© {new Date().getFullYear()} Todos los derechos reservados a RAICEP registro argentino de instituciones y homologacion de estudios profesionales.</p>
      </footer>
    </div>
  );
}
