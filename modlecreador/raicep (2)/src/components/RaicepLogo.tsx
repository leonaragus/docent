...
interface RaicepLogoProps {
  className?: string;
  size?: 'sm' | 'md' | 'lg';
  monoText?: boolean;
}

export default function RaicepLogo({ className = '', size = 'md', monoText = false }: RaicepLogoProps) {
  const iconSizeMap = {
    sm: 'w-5 h-5',
    md: 'w-8 h-8',
    lg: 'w-12 h-12'
    ...
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      <div className="text-left">
        <h1 className="font-extrabold tracking-tight text-slate-900 leading-none flex items-center gap-1.5">
          <span className="text-base sm:text-xl font-display font-black tracking-widest text-[#1e293b]">RAICEP</span>
